...
class VLLMScheduler(BaseScheduler):
    ...
    def _schedule(self) -> SchedulerOutputs:
        ...
        if buffer_1_full or buffer_2_full or self.buffer_age > self.buffer_age_threshold:
            # print(f"[VLLMScheduler._schedule] rebatching buffer is full: {self.rebatching_buffer_1} and {self.rebatching_buffer_2}. returning empty scheduler outputs.")
            self.buffer_age = 0
    ...
    def on_rebatching(self, scheduled_seq_metadata_list: List[SequenceMetadata], output_seqs: List[Sequence], ee_from_layer: int, is_flush_1: bool, is_flush_2: bool):
        ...
        for output_seq in output_seqs:
            ...
            if output_seq.seq_id in self.rebatching_buffer_1:
                if_flush_1 = True




        if is_flush_2 or (len(output_seqs) > len(scheduled_seq_metadata_list) and not if_flush_1):
            # flush 2 --> no EE
            assert ee_from_layer == -1
            for output_seq in output_seqs:
                if output_seq.seq_id in self.rebatching_buffer_2:
            ...
            if ee_from_layer == -1:
                for output_seq in output_seqs:
                    ...
                    if output_seq.seq_id in self.rebatching_buffer_1:
                        ...
                        print(f"[VLLMScheduler.on_rebatching] Warning: seq_id: {output_seq.seq_id} is not in rebatching_buffer_1")
