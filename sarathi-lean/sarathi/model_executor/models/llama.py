...
class HiddenStatesBuffer():
    ...
    def add_hidden_states(self, hidden_states: torch.Tensor, req_ids: List[int], positions: torch.Tensor) -> None:
        ...
        slots = []
        assert len(self.available_slots) >= num_hidden_states, f"Not enough slots in buffer. num_hidden_states: {num_hidden_states}, len(available_slots): {len(self.available_slots)}. hidden_states_map: {self.hidden_states_map}"
...
class LlamaModel(nn.Module):
    ...
    def forward(
        ...
        if flush_buffer:
            if self.ramp_2_type == "rebatching" and (len(self.deep_buffer_2) >= self.max_batch_size or len(self.deep_buffer_2) > len(self.deep_buffer_1)):
        ...
        for i in range(start_at_layer, len(self.layers)):
            ...
            if check_for_ee_at_ramp_2 and i == self.shallow_exit_layer_2:
                ...
                if need_skip:
                    has_ee = True
                    if not self.ramp_2_type == "rebatching":
        ...
        if has_ee:
            assert self.ramp_2_type != "rebatching"
            return hidden_states, seq_ids_in_batch, self.exited_rates, lm_logits, 2, is_flush_1, False, recompute_dict, conf, None
        else:
