...
def main():
    ...
    # --- RCT (request completion time, seconds): avg / median / tail, per policy ---
    def _rct(csv_glob):
        files = glob.glob(csv_glob)
        if not files:
            return None
        rdf = pd.read_csv(files[0])
        if "request_duration" not in rdf.columns or len(rdf) == 0:
            return None
        s = rdf["request_duration"].dropna()
        if len(s) == 0:
            return None
        return {"avg": float(s.mean()), "p50": float(s.median()),
                "p95": float(s.quantile(0.95)), "p99": float(s.quantile(0.99)), "n": int(len(s))}

    rct_rows = []
    for b in BATCHES:
        for pol, sub in [("off", f"off_b{b}"), ("median", f"median_b{b}"), ("rebatch", f"rebatch_b{b}")]:
            r = _rct(f"{R}/{sub}/req_*.csv")
            if r is not None:
                rct_rows.append({"b": b, "policy": pol, "rct_avg": r["avg"],
                                 "rct_med": r["p50"], "rct_p95": r["p95"],
                                 "rct_p99": r["p99"], "n": r["n"]})
    print()
    print("=== RCT (request completion time, seconds): avg / median(p50) / tail(p95,p99) ===")
    if rct_rows:
        rdf = pd.DataFrame(rct_rows)
        with pd.option_context("display.max_columns", None, "display.width", 200,
                               "display.float_format", lambda v: f"{v:.3f}"):
            print(rdf[["b", "policy", "rct_avg", "rct_med", "rct_p95", "rct_p99", "n"]].to_string(index=False))
    else:
        print("(no RCT data)")
